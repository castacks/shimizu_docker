...
RUN apt-get update \
 && apt-get install -y --no-install-recommends \
    wget apt-utils software-properties-common git unzip \ 
    ...
    ros-kinetic-cv-bridge ros-kinetic-pcl-conversions ros-kinetic-tf \
 && apt-get clean
...
RUN wget http://ceres-solver.org/ceres-solver-1.14.0.tar.gz \
 && tar zxf ceres-solver-1.14.0.tar.gz && rm ceres-solver-1.14.0.tar.gz \
...
# install PCL 1.8
RUN wget https://github.com/PointCloudLibrary/pcl/archive/pcl-1.8.1.zip \
 && unzip pcl-1.8.1.zip \
 && rm pcl-1.8.1.zip \
 && mv pcl-pcl-1.8.1 pcl \
 && cd pcl \
 && mkdir release \
 && cd release \
 && cmake \
